...
def init_db():
	...
	with sqlite3.connect("calendar.db") as connect: 
		...
		# スケジュール管理テーブル events
			...
			# さらに、冪等性実現のため、UNIQUEでuser_idとschedule_titleとschedule_datetimeの3つが一致するDB登録を弾く（登録ボタン連打でも予定登録は1回）
		c.execute("""
			CREATE TABLE IF NOT EXISTS events (
				...
				UNIQUE(user_id, schedule_title, schedule_datetime)
			)
		""")
...
def action_add_event():
	...
	# 3つの登録情報が一致する場合のフロントJavaScript向けアラート文（冪等性）
	except sqlite3.IntegrityError:
		return jsonify({"status": "ok", "message": "既に登録されています"})
	
	#このJSONのstatusがokなら、フロントのJavaScriptで予定追加成功とみなし、予定表示更新
...
def action_delete_event():
	...
	#ID番号は整数が期待されるので、整数に変換し、変換エラーをexceptで処理
	try:
		delete_id = int(delete_id)

	#整数型に変換できない入力があった場合のエラー処理（int()は文字列型の数字を整数型に変換する関数。TypeErrorは、delete_idが文字列型以外の場合,ValueErrorは、delete_idが文字列型だが、数字でない値の場合）
